...
class RewardTrainer:
    def __init__(self, model: RewardNet, encoder: RewardEncoder, lr: float = 1e-3):
        self.model = model
        self.encoder = encoder
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=float(lr))

    def train_step(self, samples: Sequence[RewardSample]) -> dict:
        if not samples:
            return {"loss": 0.0, "count": 0}

        batch = self.encoder.collate(samples)
        self.model.train()

        preds = []
        for noun_embedding, action_embedding in zip(batch.noun_embeddings, batch.action_embeddings):
            pred = self.model(
        ...
        targets = batch.targets.to(pred_tensor.device)
    ...
    def train_epochs(self, samples: Sequence[RewardSample], epochs: int = 1) -> list[dict]:
        history = []
        for _ in range(int(epochs)):
            history.append(self.train_step(samples))
        return history

class SubjectEventRewardTrainer:
    def __init__(self, model: SubjectEventRewardNet, encoder: RewardEncoder, lr: float = 1e-3):
        self.model = model
        self.encoder = encoder
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=float(lr))

    def train_step(self, samples: Sequence[SubjectEventRewardSample]) -> dict:
        if not samples:
            return {"loss": 0.0, "count": 0}
    ...
    def train_epochs(self, samples: Sequence[SubjectEventRewardSample], epochs: int = 1) -> list[dict]:
        history = []
        for _ in range(int(epochs)):
            history.append(self.train_step(samples))
        return history
